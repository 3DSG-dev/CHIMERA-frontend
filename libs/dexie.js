...
function Dexie(dbName, options) {
    ...
    if (options && options["storageOption"])
    {
        storageOption = options["storageOption"];
    }
    var opts = extend({
        // Default Options
        addons: Dexie.addons,
    ...
    */
    ...
    this.open = function () {
        ...
        return Promise.race([openCanceller, new Promise(function (resolve, reject) {
            ...
            var req = autoSchema ? indexedDB.open(dbName, { storage: storageOption }) : indexedDB.open(dbName, { storage: storageOption, version: Math.round(db.verno * 10)});
