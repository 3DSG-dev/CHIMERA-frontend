<?php
    ...
    if (!IsNullOrEmptyString($codiceOggetto) && !IsNullOrEmptyString($codiceCampo)) {
        if (IsNullOrEmptyString($valore)) {
            $SQL = "SELECT setoggettiversioniinfoschedacombovalue($codiceOggetto, $codiceCampo, null)";
        }
        else {
            $valore = pg_escape_string($valore);
            $SQL = "SELECT setoggettiversioniinfoschedacombovalue($codiceOggetto, $codiceCampo, $valore)";
        }
    }
    else {
