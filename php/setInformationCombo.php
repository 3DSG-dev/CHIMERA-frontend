<?php
    ...
    if (!IsNullOrEmptyString($codiceRiferimento) && !IsNullOrEmptyString($codiceCampo)) {
        if (IsNullOrEmptyString($valore)) {
            $SQL = "SELECT set" . $dbReference . "infoSchedaComboValue($codiceRiferimento, $codiceCampo, null)";
        }
        else {
            $valore = pg_escape_string($valore);
            $SQL = "SELECT set" . $dbReference . "infoSchedaComboValue($codiceRiferimento, $codiceCampo, $valore)";
        }
    }
    else {
