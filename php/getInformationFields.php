<?php
    ...
    $SQL = 'SELECT "OggettiVersion_ListaSchede"."Titolo", "OggettiVersion_ListaInformazioni".* FROM "OggettiVersion_ListaSchede" JOIN "OggettiVersion_ListaInformazioni" ON "OggettiVersion_ListaSchede"."Codice" = "OggettiVersion_ListaInformazioni"."CodiceTitolo" ORDER BY "OggettiVersion_ListaSchede"."Posizione", "OggettiVersion_ListaInformazioni"."Posizione", "OggettiVersion_ListaInformazioni"."Campo"';
    $result = pg_query($dbConnection, $SQL) or die ("Error: $SQL");
    $rowArray = array();
    while ($row = pg_fetch_array($result, null, PGSQL_ASSOC)) {
        $rowArray[] = $row;
