<?php
    ...
    if (!IsNullOrEmptyString($codiceOggetto) && !IsNullOrEmptyString($codiceCampo)) {
        if (IsNullOrEmptyString($valore)) {
            $SQL = "SELECT setoggettiinfoschedavalue($codiceOggetto, $codiceCampo, null)";
        }
        else {
            $valore = pg_escape_literal($valore);
            $SQL = "SELECT setoggettiinfoschedavalue($codiceOggetto, $codiceCampo, to_timestamp($valore, 'DD/MM/YY, HH24:MI:SS'))";
        }
    }
    else {
